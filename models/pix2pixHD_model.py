...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks_DMSN.define_G(netG_input_nc, opt.output_nc, opt.ngf, opt.netG,
        ...
        if self.isTrain:
            ...
            if not opt.no_instance:
                netD_input_nc += 1
            self.netD = networks_DMSN.define_D(netD_input_nc, opt.ndf, opt.n_layers_D, opt.norm, use_sigmoid,
                                               opt.num_D, not opt.no_ganFeat_loss, gpu_ids=self.gpu_ids)

        ### Encoder network
        if self.gen_features:
            self.netE = networks_DMSN.define_G(opt.output_nc, opt.feat_num, opt.nef, 'encoder',
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks_DMSN.GANLoss(use_lsgan=not opt.no_lsgan, tensor=self.Tensor)
            ...
            if not opt.no_vgg_loss:
                self.criterionVGG = networks_DMSN.VGGLoss(self.gpu_ids)
